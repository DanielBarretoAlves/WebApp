...
//Draw the pie chart for ESP when chart is loaded

google.charts.setOnLoadCallback(drawEspChart);
...
function drawEspChart(){
    espAprovados = localStorage.getItem("espAprovados");
    espReprovado = localStorage.getItem("espReprovados");
    let x =espAprovados;
    let y = espReprovado;
    var data = new google.visualization.DataTable();
    data.addColumn('string', 'Topping');
    ...
    var chart = new google.visualization.PieChart(document.getElementById('espData'));
    chart.draw(data, options);
}